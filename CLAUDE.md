...
# Legacy install (CLI-only, no API/UI)
...
# Database CLI — ingest, query, manage
python cli.py ingest paper.pdf -v --tag "energy"
python cli.py ingest ./papers/ --recursive
python cli.py list [--tag TAG] [--json]
python cli.py show <id> [--sections] [--references] [--citations] [--llm] [--json]
python cli.py search "query" [--field title|abstract|author|all]
python cli.py stats
python cli.py export -o papers.json
python cli.py tag add <id> <name>
python cli.py delete <id> --force

# Phase 2: LLM analysis
python cli.py analyze <id>               # analyze one paper
python cli.py analyze --missing          # analyze all papers without llm_analyzed_at
python cli.py analyze --all --force      # re-analyze every paper
python cli.py analyze <id> --skip-refs --skip-lit-review  # paper summary only
python cli.py analyze <id> --dry-run     # preview without LLM calls

# Phase 3: embeddings (Qwen text-embedding-v4)
python cli.py ingest paper.pdf --embed   # ingest + embed in one shot
python cli.py embed <id>                 # embed one paper
python cli.py embed --missing            # embed papers without title/abstract emb
python cli.py embed --all --force        # re-embed everything
python cli.py embed <id> --skip-refs     # skip per-reference embeddings

# Phase 3: three-tier retrieval (multi-path → RRF → LLM rerank)
python cli.py search-rag "EV energy consumption"            # default: hybrid mode
python cli.py search-rag "电动汽车续航" --mode vector        # semantic only (cross-lingual)
python cli.py search-rag "MILP charging stations" --mode fts # lexical (BM25) only
python cli.py search-rag "query" --rerank --top-k 10 --explain
python cli.py search-rag "..." --tag energy --year-from 2024

# Phase 3: citation cross-analysis
python cli.py cross-analyze detect                 # find shared refs (DOI + title)
python cli.py cross-analyze detect --with-embedding  # also fuzzy match via ref_embedding
python cli.py cross-analyze relate --min-shared 2    # LLM-characterize pair relationships
python cli.py cross-analyze all                      # detect + relate in one shot
python cli.py cross-analyze list --min-shared 2      # top pairs by shared-ref count
python cli.py cross-analyze pair <a> <b>             # inspect a specific pair

# Phase 4: multi-agent assistant
python cli.py agent "<question>"                            # orchestrator dispatches specialists
python cli.py agent "<question>" --task lit-review          # thematic review
python cli.py agent "<question>" --task qa                  # cited answer
python cli.py agent "<question>" --task compare             # multi-paper comparison table
python cli.py agent "<question>" --task gap                 # research-gap analysis
python cli.py agent "<question>" --show-plan --show-trace   # debug orchestration
python cli.py agent "<question>" --no-synth                 # see raw specialist JSON outputs

# Override database path (works on any subcommand)
python cli.py --db /path/to/my.db list

# Environment variables
PAPER_DB_PATH=/path/to/papers.db        # override default data/papers.db
...
No test suite exists yet. No packaging config (pyproject.toml/setup.py). Not a git repository.
...
```
cli.py → BatchProcessor → PaperExtractor.extract()
...
- `db/paper_repo.py` — transactional insert of paper + sections + references + citation_locations. CASCADE delete.
...
- `schema.py` — 9 core tables + 3 layout-aware sibling tables (`section_tables` / `section_formulas` / `section_images`, only populated by the MinerU backend). `papers.parse_backend` records which extractor produced each row. Schema versioned via `schema_version` table — bumped to v6 for the rich-content tables.
...
1. **`analyze_paper(paper, sections)`** → fills `papers.llm_summary / llm_research_field / llm_methodology / llm_key_findings`. Uses complex tier. Picks Intro/Methods/Results/Conclusion sections by keyword.
...
`BatchProcessor` exposes both an in-line path (`ingest --embed`) and a standalone path (`embed_paper_by_id`). The CLI `embed` subcommand handles single / `--all` / `--missing` / `--force` / `--dry-run`, and `cli.py stats` shows embedding population per table.
...
Driven via `cli.py search-rag`. Auto-creates FTS index on first use.
...
Driven via `cli.py cross-analyze {detect, relate, all, list, pair}`.
...
| Module | Role |
|---|---|
| `agents/tools.py` | 7 read-only tools (`search_papers`, `get_paper`, `get_paper_sections`, `get_lit_review_entries`, `get_paper_references`, `find_related_papers`, `search_within_paper`) + OpenAI function-calling schemas + `ToolContext` (db, llm, embedder) |
| `agents/base.py` | `BaseAgent` — generic function-calling loop. DeepSeek thinking mode: passes `reasoning_content` back through messages on multi-turn tool use (per DeepSeek docs) |
...
| `agents/orchestrator.py` | Plans (one v4-pro thinking call → JSON plan) and dispatches specialists in parallel; results passed to Synth |
| `agents/synth.py` | Writes the final markdown report (executive summary + section structure + inline `[paper N]` citations + Sources line) |
...
Driven via `cli.py agent`. `--task auto` (default) runs orchestrator; `--task lit-review|qa` calls one specialist directly. `--show-plan`/`--show-trace` for debugging.
...
DB layer records: `PaperRecord`, `SectionRecord`, `ReferenceRecord`, `CitationLocationRecord`, `TagRecord` (models.py). JSON list/dict fields use `json.dumps`/`json.loads` in `to_row()`/`from_row()`.
...
Phase 2 (done): LLM content analysis — DeepSeek/OpenAI-compatible provider, three-stage `cli.py analyze` (paper summary, ref scoring, lit-review viewpoints)
Phase 3 (done):
  - Embeddings: Qwen text-embedding-v4 via DashScope, `cli.py embed`, float32 BLOB storage
  - Retrieval: FTS5 + vector + RRF fusion + LLM rerank, `cli.py search-rag`
  - Citation cross-analysis: DOI + title + (optional) embedding detection → LLM pair relationship, `cli.py cross-analyze`
Phase 4 (done):
  - Orchestrator (v4-pro thinking, JSON plan) + 4 specialists (lit_review, qa, compare, gap) + Synth, parallel dispatch via ThreadPoolExecutor, `cli.py agent`
Phase 5 (done):
